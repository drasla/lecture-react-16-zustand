...
function TodoPage() {
    ...
    return (
        <div className={twMerge(["max-w-2xl", "w-full", "mx-auto", "mt-10", "p-4"])}>
            <h1
                className={twMerge(
                    ["mb-8"],
                    ["text-3xl", "font-bold", "text-center", "text-primary-main"],
                )}>
                My Tasks
            </h1>
            ...
            <form className={twMerge(["flex", "gap-2", "mb-8"])} onSubmit={onSubmit}>
                <input
                    ...
                    className={twMerge(
                        ["flex-1", "p-3"],
                        ["rounded-lg", "border", "border-divider", "bg-background-paper"],
                        ["focus:border-primary-main", "focus:border-2", "outline-none"],
                    )}
                />
                <Button variant={"primary"} className={twMerge("p-3", "py-6.5")}>
                    <FiPlus size={24} />{" "}
                    <span className={twMerge(["hidden", "sm:inline"])}>Add</span>
                </Button>
            </form>

            <div className={"space-y-3"}>
                {todos.length === 0 && (
                    <p className={twMerge(["mt-10", "text-center", "text-text-disabled"])}>
                        할 일이 없습니다. 새로 추가해보세요!
                    </p>
                )}
                {myTodos &&
