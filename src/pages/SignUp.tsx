...
function SignUp() {
    ...
    const onSubmit = (data: SignUpFormValues) => {
        ...
        // string을 객체(또는 배열)로 변환 : JSON.parse()
        ...
        // 중복 체크를 하는 방법은, Array 안에 있는 데이터가 지금 입력된 값이 있나 없나
        // 확인해야 함. 확인은 some(판별하는함수) 메서드를 사용
        // 판별하는 함수 :
        // (순회하는요소) => (순회하는요소의 email이 지금 입력된 data.email과 같거나,
        ...
        if (isDuplicate) {
            setError("root", { message: "이미 등록된 이메일 또는 아이디가 존재합니다." });
            return;
        }
    ...
    return (
        <div
            className={twMerge(
                ...
                ["p-4", "w-full", "min-h-[calc(100dvh-4rem)]"],
            )}>
            <div
                className={twMerge(
                    ["w-full", "p-8"],
                    ["bg-background-paper", "border", "border-divider", "rounded-xl", "shadow-lg"],
                )}>
                <h2
                    className={twMerge([
                        "text-2xl",
                ...
                <form onSubmit={handleSubmit(onSubmit)} className={twMerge("space-y-4")}>
                    <Input
                        label={"ID"}
                        registration={register("userId", { required: "아이디는 필수값입니다." })}
                        error={errors.userId}
                        type={"text"}
                    ...
                    <Input
                        label={"이름"}
                        registration={register("name", { required: "이름은 필수값입니다." })}
                        error={errors.name}
                        type={"text"}
                    ...
                    {errors.root && (
                        <div className={twMerge(["text-sm", "text-center", "text-error-main"])}>
                            {errors.root.message}
                        </div>
                    )}
